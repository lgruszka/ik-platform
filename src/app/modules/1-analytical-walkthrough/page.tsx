  ...
  return (
    <>
      ...
      <div className="px-8 py-8 max-w-5xl space-y-8">
        ...
        <section className="prose-ik">
          ...
          <div className="not-prose my-4">
            <CommonsImage
              ...
              alt="PUMA 560 — rzeczywisty manipulator z nadgarstkiem sferycznym"
              caption="Forma A warunku Piepera w praktyce: trzy ostatnie osie obrotu nadgarstka manipulatora PUMA 560 przecinają się fizycznie w jednym punkcie wewnątrz obudowy nadgarstka. Dzięki temu obroty q₄, q₅, q₆ zmieniają tylko orientację narzędzia, nie przesuwając środka nadgarstka."
          ...
          <div className="rounded-lg border border-[var(--panel-border)] bg-[var(--panel)] px-5 py-4 my-4 not-prose">
            <p className="font-semibold mb-2">Pomocna metafora — Puma jako dwa „podroboty" zmontowane szeregowo</p>
            <p className="text-sm text-[var(--foreground)] mb-2">
              Zamiast od razu liczyć, warto na minutę spojrzeć na manipulator
              jako na dwa logicznie niezależne urządzenia:
            </p>
            <ul className="text-sm list-disc pl-5 space-y-1 text-[var(--foreground)]">
              <li>
                <strong>Podrobot pozycjonujący</strong> — 3-osiowe ramię
                (podstawa + bark + łokieć, czyli <M tex="q_1, q_2, q_3" />),
                kończące się „kulką" w punkcie przecięcia trzech ostatnich osi.
                Jego jedynym zadaniem jest <em>umieścić tę kulkę</em> w żądanym
                punkcie <M tex="\mathbf{p}_\mathrm{wc}" /> przestrzeni.
              </li>
              <li>
                <strong>Podrobot orientujący</strong> — 3-osiowy nadgarstek
                sferyczny (<M tex="q_4, q_5, q_6" />) zamontowany w tej samej
                kulce. Jego zadaniem jest <em>obrócić</em> narzędzie do żądanej
                orientacji <em>nie ruszając kulką</em>, bo wszystkie jego trzy
                osie obrotu przechodzą dokładnie przez nią.
              </li>
            </ul>
            <p className="text-sm text-[var(--foreground)] mt-3">
              To rozdzielenie nie jest tylko opisem — jest{" "}
              <em>matematycznym faktem</em>: pochodna pozycji wrist centre
              względem <M tex="q_4, q_5, q_6" /> jest zerowa, bo obroty wokół
              osi przechodzących przez punkt nie przesuwają tego punktu. Dlatego
              można najpierw rozwiązać podproblem pozycji (3 niewiadome, 3
              równania skalarne), a dopiero potem podproblem orientacji
              (kolejne 3 niewiadome z warunku, że pełna macierz rotacji ma być
              taka jak zadano). Bez tego rozdzielenia musielibyśmy
              jednocześnie zmagać się z układem 6 równań nieliniowych, dla
              którego analityczne rozwiązanie istnieje, ale jest dramatycznie
              bardziej skomplikowane (równanie 16-tego stopnia w metodzie
              Raghavana–Rotha dla geometrii nie-pieperowskich).
            </p>
          </div>
        </section>
        ...
        <section className="prose-ik">
          <h2>Co fizycznie robi każdy z sześciu przegubów</h2>
          <p>
            Zanim zanurzymy się w algebrę, ułóżmy w głowie obraz tego, jak
            ramię „pracuje". Każdy przegub Pumy ma jedną dokładnie wyznaczoną
            funkcję mechaniczną — i dla każdej istnieje codzienna analogia,
            która ułatwia zapamiętanie. To krótkie wprowadzenie zwraca się z
            nawiązką, gdy później pojawi się pytanie „dlaczego{" "}
            <M tex="q_3" /> kontroluje wysokość, a <M tex="q_5" /> nie".
          </p>
          <div className="not-prose grid grid-cols-1 md:grid-cols-2 gap-3 my-4">
            <JointRolePanel
              index="q₁"
              name="Swivel podstawy"
              axis="pionowa oś bazy (ẑ₀)"
              analogy="Dolny obrót żurawia portowego — cała wieża obraca się wokół własnej pionowej osi."
            >
              Decyduje wyłącznie o <strong>azymucie</strong> — w którą stronę
              świata zwrócone jest ramię. Nie zmienia ani wysokości, ani
              wysunięcia TCP, jedynie obraca całą resztę manipulatora. Z
              perspektywy IK: ten staw widzimy w „rzucie z góry" — bierzemy{" "}
              <M tex="(p_x, p_y)" /> docelowej pozycji i pytamy: w którą stronę
              ramię musi się skręcić, żeby w ogóle „móc patrzeć" na cel.
            </JointRolePanel>

            <JointRolePanel
              index="q₂"
              name="Bark — pochylenie ramienia"
              axis="pozioma, prostopadła do ramienia"
              analogy="Podnosisz wyprostowaną rękę w górę lub w dół — bez zginania łokcia. To dokładnie q₂."
            >
              Wraz z <M tex="q_3" /> kontroluje punkt 2D, w który sięga
              końcówka ramienia (wysokość + wysunięcie radialne) — w
              płaszczyźnie pionowej wyznaczonej już przez <M tex="q_1" />.
              Czysto „pionowy" stopień swobody barku.
            </JointRolePanel>

            <JointRolePanel
              index="q₃"
              name="Łokieć — zgięcie przedramienia"
              axis="równoległa do osi barku"
              analogy="Zginasz własną rękę w łokciu — dłoń przybliża się do barku, choć ramię się nie obraca."
            >
              Decyduje o „rozłożeniu" całego ramienia: im bardziej wyciągnięte,
              tym dalej sięga; im bardziej zgięte, tym wyżej/bliżej.
              Geometrycznie odpowiada za <em>długość przekątnej</em>{" "}
              bark→wrist center (poprzez prawo cosinusów — patrz krok 4).
              Tutaj pojawia się słynna dwuznaczność „elbow up / elbow down".
            </JointRolePanel>

            <JointRolePanel
              index="q₄"
              name="Roll przedramienia"
              axis="wzdłuż osi przedramienia"
              analogy="Obracasz nadgarstek trzymając śrubokręt — sama dłoń kręci się wokół osi narzędzia."
            >
              <strong>Pierwszy</strong> z trzech stawów nadgarstka. Nie zmienia
              <em>pozycji</em> środka nadgarstka, bo oś obrotu przechodzi
              dokładnie przez ten punkt. Wpływa za to na orientację — to
              właśnie ten staw „skręca" całą głowę nadgarstka wokół przedramienia.
            </JointRolePanel>

            <JointRolePanel
              index="q₅"
              name="Pitch nadgarstka"
              axis="prostopadła do z₄"
              analogy="Zginasz dłoń w nadgarstku — od pozycji wyprostowanej (ramię w jednej linii z dłonią) do pełnego zgięcia."
            >
            ...
            <JointRolePanel
              index="q₆"
              name="Roll TCP"
              axis="wzdłuż osi narzędzia (ẑ₆)"
              analogy="Klucz nasadowy włożony w nakrętkę — kierunek wskazany przez klucz ustalają q₄ i q₅, ale obrót klucza wokół własnej osi to dokładnie q₆."
            >
              Końcowy stopień swobody — orientuje narzędzie wokół jego własnej
              osi po tym, jak <M tex="q_4, q_5" /> ustawiły kierunek, w który
              narzędzie „patrzy". Bez <M tex="q_6" /> robot mógłby tylko
              wskazywać kierunek; z nim — może też ustawić orientację gripper'a,
              szczęk czy dyszy.
            </JointRolePanel>
          </div>
          <p>
            <strong>Dlaczego trzy + trzy?</strong> Trzy pierwsze stawy
            (<M tex="q_1, q_2, q_3" />) ustalają <em>pozycję</em> 3-wymiarowego
            punktu (środek nadgarstka) — bo 3 niewiadome wystarczają, żeby
            opisać dowolny punkt w <M tex="\mathbb{R}^3" />. Trzy ostatnie
            (<M tex="q_4, q_5, q_6" />) ustalają <em>orientację</em> w
            przestrzeni — bo grupa obrotów <M tex="SO(3)" /> jest również
            3-wymiarowa. To nie przypadek — to inżynierski projekt
            optymalizujący liczbę stopni swobody pod konkretne zadanie:
            ustawić narzędzie w pełnej dowolności w <M tex="SE(3)" /> przy
            możliwie najprostszej geometrii.
          </p>
        </section>
...
const [px, py, pz] = extractPosition(T06);  // ostatnia kolumna`}</code></pre>
        ...
        <StepPanel number={2} title="Wyznaczenie q₁ — dwie gałęzie barku">
          <div className="rounded-lg border border-[var(--panel-border)] bg-[var(--panel)] px-4 py-3 my-4 not-prose">
            <p className="text-sm font-semibold mb-2">
              Intuicja: cień TCP na podłodze
            </p>
            <p className="text-sm text-[var(--foreground)] mb-2">
              Wyobraź sobie, że nad TCP świeci pionowa lampa rzucająca cień na
              poziomą płaszczyznę podstawy. Ten cień ma w bazie współrzędne{" "}
              <M tex="(p_x, p_y, 0)" />. Naturalna pierwsza myśl: <em>obrót
              podstawy musi po prostu skierować ramię w stronę cienia</em> — czyli{" "}
              <M tex="q_1 = \operatorname{atan2}(p_y, p_x)" />.
            </p>
            <p className="text-sm text-[var(--foreground)] mb-2">
              Dla manipulatora <em>bez odsadzenia</em> tak właśnie by było.
              Puma ma jednak <M tex="d_3 \approx 12{,}5\,\mathrm{cm}" /> —
              <em> boczne odsunięcie</em> osi przedramienia od osi ramienia.
              Skutkiem tego, gdy bark obraca się o kąt <M tex="q_1" />,
              przedramię (a w konsekwencji środek nadgarstka) <em>nie</em>{" "}
              znajduje się ściśle przed barkiem — leży lekko obok, w bok od osi
              ramienia o stałą odległość <M tex="d_3" />.
            </p>
            <p className="text-sm text-[var(--foreground)] mb-2">
              Z perspektywy „cienia na podłodze" oznacza to, że cień nie leży
              dokładnie na linii barku — jest przesunięty w bok. Stąd potrzeba
              drugiego członu: <M tex="q_1 = \operatorname{atan2}(p_y, p_x) - \operatorname{atan2}(d_3, \rho)" />.
              Pierwszy <code>atan2</code> wskazuje na cień, drugi koryguje o ten
              boczny offset (dla typowych pozycji ~7°). Te 7° to <em>dokładnie</em>{" "}
              błąd, jaki popełniają studenckie kody, w których pominięto drugi człon.
            </p>
            <p className="text-sm text-[var(--foreground)]">
              Geometrycznie: wektor (<M tex="\rho, d_3" />) w obróconym układzie
              ramienia to ten sam wektor co (<M tex="p_x, p_y" />) w bazie —
              tylko obrócony o <M tex="q_1" />. Wektor (<M tex="\rho, d_3" />){" "}
              ma <em>nachylenie</em>{" "}
              <M tex="\operatorname{atan2}(d_3, \rho)" /> względem osi ramienia,
              a wektor (<M tex="p_x, p_y" />) ma nachylenie{" "}
              <M tex="\operatorname{atan2}(p_y, p_x)" /> względem osi X bazy.
              Różnica tych dwóch nachyleń to właśnie kąt obrotu między układami,
              czyli <M tex="q_1" />.
            </p>
          </div>
          <p>
...
const q1    = phi - Math.atan2(D3, rho);                    // q₁ na obu gałęziach`}</code></pre>
        ...
        <StepPanel number={4} title="Wyznaczenie q₃ — twierdzenie cosinusów, dwie gałęzie łokcia">
          <div className="rounded-lg border border-[var(--panel-border)] bg-[var(--panel)] px-4 py-3 my-4 not-prose">
            ...
            <p className="text-sm text-[var(--foreground)] mb-2">
              Kluczowa obserwacja, która sprawia, że <em>można w ogóle</em>{" "}
              w tym momencie szukać <M tex="q_3" /> w izolacji od reszty:
              <strong>{" "}odległość |bark → wrist center| nie zależy od{" "}
              <M tex="q_1" /> ani <M tex="q_2" /></strong> — zależy
              wyłącznie od <M tex="q_3" />.
            </p>
            <p className="text-sm text-[var(--foreground)] mb-2">
              Intuicyjnie: <M tex="q_1" /> i <M tex="q_2" /> obracają <em>cały
              trójkąt</em> bark-łokieć-wrist jako sztywne ciało — pierwszy
              wokół pionowej osi bazy, drugi wokół osi barku. Obroty sztywne
              nie zmieniają długości żadnego boku trójkąta. To, co <em>zmienia</em>{" "}
              wewnętrzny kształt trójkąta — a w szczególności długość
              przekątnej bark↔wrist — to wyłącznie zgięcie w łokciu, czyli{" "}
              <M tex="q_3" />.
            </p>
            <p className="text-sm text-[var(--foreground)]">
              Stąd ważny wniosek metodologiczny: mając zadaną pozycję wrist
              centre (a więc znaną przekątną <M tex="D = \sqrt{\rho^2 + p_z^2}" />),
              możemy <em>natychmiast</em> wyłuskać <M tex="q_3" /> z jednego
              równania skalarnego, <em>nie wiedząc</em> jeszcze ile wynosi{" "}
              <M tex="q_2" />. Dopiero potem wrócimy do dwuwymiarowego problemu
              i wyznaczymy <M tex="q_2" />, traktując kształt trójkąta jako już
              ustalony. To dlatego algorytm liczy stawy w kolejności{" "}
              <M tex="q_1 \to q_3 \to q_2" />, a nie w kolejności indeksów.
            </p>
          </div>
          <p>
          ...
          <div className="rounded-lg border-l-4 border-purple-500 bg-purple-50 dark:bg-purple-950/20 px-4 py-3 my-4 not-prose">
            <p className="text-sm font-semibold mb-1">
              Obraz pomocniczy — naprężona linka i dwa lustrzane trójkąty
            </p>
            <p className="text-sm text-[var(--foreground)]">
              Wyobraź sobie naprężoną linkę o długości{" "}
              <M tex="D" />, której końce są przymocowane w barku{" "}
              <M tex="A" /> i w środku nadgarstka <M tex="W" />. Łokieć{" "}
              <M tex="E" /> jest swobodnym przegubem o ustalonej odległości{" "}
              <M tex="a_2" /> od barku i <M tex="L" /> od wrist — może
              „opaść" po dowolnej stronie linki. Daje to <em>dwa
              identycznie wymierzone trójkąty</em> będące względem siebie
              lustrzanym odbiciem przez bok <M tex="D" />:
            </p>
            <ul className="text-sm list-disc pl-5 space-y-1 mt-2 text-[var(--foreground)]">
              <li>
                <strong>elbow up</strong> — łokieć powyżej linii bark↔wrist,
                ramię „zgięte w górę" jak u człowieka podnoszącego rękę.
              </li>
              <li>
                <strong>elbow down</strong> — łokieć poniżej, ramię „złożone w
                dół" jak u robota chwytającego coś z poziomu stołu.
              </li>
            </ul>
            <p className="text-sm text-[var(--foreground)] mt-2">
              Obie konfiguracje dają <em>dokładnie</em> tę samą pozycję wrist
              centre, ale wymagają różnych <M tex="q_2, q_3" />. Stąd
              binarny wybór znaku <M tex="\pm\sqrt{\,\cdot\,}" /> w
              wyrażeniu na <M tex="\sin\gamma" /> — to przełącznik między
              dwoma lustrzanymi obrazami tego samego trójkąta.
            </p>
          </div>
          <p>
...
// ... pozostałe 8 elementów analogicznie`}</code></pre>
        ...
        <StepPanel number={7} title="Ekstrakcja q₄, q₅, q₆ z macierzy R₃⁶">
          <p>
            ...
            typu ZYZ</strong>: dwa skręcenia osi <M tex="\pm\pi/2" /> między
          ...
          <div className="rounded-lg border border-[var(--panel-border)] bg-[var(--panel)] px-4 py-3 my-4 not-prose">
            ...
            <p className="text-sm text-[var(--foreground)] mb-2">
              Część rotacyjna każdej macierzy DH (Craig) ma postać{" "}
              <M tex="R_x(\alpha_{i-1})\,R_z(\theta_i)" /> — najpierw skręcamy
              poprzednią oś <M tex="\hat z" /> o stały kąt <M tex="\alpha_{i-1}" />{" "}
              (geometria ogniwa), potem obracamy o zmienną <M tex="\theta_i = q_i" />{" "}
              wokół nowej osi <M tex="\hat z_i" /> (kąt przegubu).
              Sklejając trzy ostatnie ogniwa, dostajemy:
            </p>
            <MathBlock tex="R_3^6 \;=\; \underbrace{R_z(q_4)}_{\text{przegub 4}}\,\underbrace{R_x(\alpha_4)\,R_z(q_5)}_{\text{ogniwo 4{\to}5}}\,\underbrace{R_x(\alpha_5)\,R_z(q_6)}_{\text{ogniwo 5{\to}6}}" />
            <p className="text-sm text-[var(--foreground)] mb-2">
              Pierwsza cegiełka to czyste <M tex="R_z(q_4)" /> bo skręcenie{" "}
              <M tex="\alpha_3 = -\pi/2" /> zostało już „pochłonięte" w macierzy{" "}
              <M tex="R_0^3" />. Pozostałe dwie pary biorą skręcenia z tabeli DH —{" "}
              <M tex="\alpha_4 = +\pi/2" /> (oś <M tex="\hat z_4" /> obraca się o
              90° aby pokryć się z osią <M tex="\hat z_5" /> prostopadłą do
              przedramienia) oraz <M tex="\alpha_5 = -\pi/2" /> (oś <M tex="\hat z_5" />{" "}
              wraca o 90° w przeciwną stronę, aby oś <M tex="\hat z_6" /> wskazywała
              wzdłuż narzędzia). Stąd „kanapka" <em>z–x–z–x–z</em>: trzy obroty
              przegubowe <M tex="R_z(q_4), R_z(q_5), R_z(q_6)" /> przeplatane dwoma
              stałymi „skręceniami montażowymi" <M tex="R_x(\pm\pi/2)" />.
            </p>
            <p className="text-sm text-[var(--foreground)] mb-0">
              <strong>Dlaczego to się nazywa „typ ZYZ"?</strong> Bo gdy
              odpowiednio przenieść stałe macierze <M tex="R_x(\pm\pi/2)" /> przez
              obroty <M tex="R_z" /> (korzystając z tożsamości{" "}
              <M tex="R_x(\pi/2)\,R_z(\theta)\,R_x(-\pi/2) = R_y(\theta)" />), cała
              kanapka redukuje się do <M tex="R_z(q_4)\,R_y(q_5)\,R_z(q_6 + \mathrm{const})" /> —
              klasycznych kątów Eulera ZYZ. Konwencja DH wymusza zapis przez{" "}
              <M tex="R_z" /> i <M tex="R_x" />, ale geometria pod spodem to{" "}
              dokładnie te same trzy obroty z–y–z, których uczy się w kinematyce
              ciała sztywnego.
            </p>
          </div>
          <p>
          ...
          <div className="rounded-lg border border-[var(--panel-border)] bg-[var(--panel)] px-4 py-3 my-4 not-prose">
            ...
            <p className="text-sm text-[var(--foreground)] mb-2">
              Mamy już <M tex="\cos q_5 = R_3^6[1][2]" /> — to wystarczyłoby do
              policzenia <M tex="q_5" /> przez <code>acos</code>, ale dwa istotne
              powody każą sięgnąć po <code>atan2</code>: (1) tracimy informację o
              znaku sinusa, więc nie odróżnimy gałęzi <em>no-flip</em> od{" "}
              <em>flip</em>; (2) <code>acos</code> jest źle uwarunkowany w pobliżu
              <M tex="\pm 1" /> (małe błędy zaokrąglenia → duże błędy kąta).
              Dlatego rekonstruujemy także <M tex="\sin q_5" />.
            </p>
            <p className="text-sm text-[var(--foreground)] mb-2">
              Z dwóch równości z pierwszej kolumny środkowego wiersza:
            </p>
            <MathBlock tex="R_3^6[1][0] = s_5\,c_6, \qquad R_3^6[1][1] = -s_5\,s_6" />
            <p className="text-sm text-[var(--foreground)] mb-2">
              Podnosimy do kwadratu i dodajemy, korzystając z{" "}
              <em>jedynki trygonometrycznej</em> <M tex="c_6^2 + s_6^2 = 1" />:
            </p>
            <MathBlock tex="R_3^6[1][0]^2 + R_3^6[1][1]^2 \;=\; s_5^2\,c_6^2 + s_5^2\,s_6^2 \;=\; s_5^2\,(c_6^2 + s_6^2) \;=\; s_5^2" />
            <p className="text-sm text-[var(--foreground)] mb-2">
              Stąd <M tex="\sin q_5 = \pm\sqrt{R_3^6[1][0]^2 + R_3^6[1][1]^2}" /> —{" "}
              znak <M tex="\pm" /> jest fundamentalnie nierozstrzygalny z tych
              równań (układ trzech równań na trzy niewiadome ma dwie gałęzie
              rozwiązań). Mając parę <M tex="(\sin q_5, \cos q_5)" />, składamy:
            </p>
            <MathBlock tex="q_5 \;=\; \operatorname{atan2}(\sin q_5,\;\cos q_5) \;=\; \operatorname{atan2}\!\left(\pm\sqrt{R_3^6[1][0]^2 + R_3^6[1][1]^2},\;R_3^6[1][2]\right)" />
            <p className="text-sm text-[var(--foreground)] mb-0">
          ...
          <h3>Osobliwość (singularność) nadgarstka (q₅ ≈ 0)</h3>
          <p>
...
if (sq5_abs < eps) {
  ...
  for (const wristSign of [+1, -1]) {
    ...
    const q6 = Math.atan2(-wristSign*R36_11,  wristSign*R36_10);
        ...
        <section className="prose-ik">
          <h2>Klucz przewodni — pomysły algebraiczne, nie wzory</h2>
          <p>
            Cała ściąga, którą zobaczysz za chwilę, to <em>litera</em>{" "}
            algorytmu — to, co trzeba <em>wpisać</em> do kodu. Ale za każdym
            wzorem stoi <em>pomysł algebraiczny</em>: jakaś sprytna kombinacja,
            która pozwala wyłuskać pojedynczą niewiadomą z układu wielu równań.
            Jeśli zapamiętasz pomysły, formuły zrekonstruujesz sam; jeśli
            zapamiętasz tylko formuły, przy pierwszym robocie o innej geometrii
            będziesz musiał zaczynać od zera.
          </p>
          <table>
            <thead>
              <tr>
                <th>Staw</th>
                <th>Główny pomysł</th>
                <th>Technika algebraiczna</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td><M tex="q_1" /></td>
                <td>Rzut na płaszczyznę poziomą eliminuje wszystkie wyższe stawy</td>
                <td>
                  Dwa <code>atan2</code> w różnicy — kąt do celu w XY{" "}
                  <em>minus</em> kąt korygujący offset <M tex="d_3" />.
                </td>
              </tr>
              <tr>
                <td><M tex="q_3" /></td>
                <td>
                  Odległość bark↔wrist jest niezmiennikiem obrotów{" "}
                  <M tex="q_1, q_2" /> — zależy tylko od <M tex="q_3" />.
                </td>
                <td>
                  Prawo cosinusów na trójkącie bark-łokieć-wrist; jedno
                  równanie skalarne <M tex="K = f(q_3)" />.
                </td>
              </tr>
              <tr>
                <td><M tex="q_2" /></td>
                <td>
                  Po wyznaczeniu <M tex="q_3" /> układ pozycji staje się
                  liniowy w (<M tex="c_2, s_2" />).
                </td>
                <td>
                  Pogrupowanie wyrazów pozycji wg <M tex="c_2" /> i{" "}
                  <M tex="s_2" /> daje układ liniowy 2×2; reguła Cramera
                  zwraca obie wartości i <code>atan2(s_2, c_2)</code> dopina
                  kąt.
                </td>
              </tr>
              <tr>
                ...
                <td>
                  Mając <M tex="q_1, q_2, q_3" />, „brakująca" rotacja{" "}
                  <M tex="R_3^6" /> jest jawnie znana — można z niej odczytać
                  trzy kąty Eulera.
                </td>
                <td>
                  <M tex="R_3^6 = (R_0^3)^{\!\top}\,R" />, a następnie
                  porównanie konkretnych elementów macierzy z postacią
                  symboliczną iloczynu trzech rotacji wokół osi nadgarstka.
                </td>
              </tr>
            </tbody>
          </table>
          <p>
            <strong>Wspólny wzorzec</strong>: każdy staw odkrywa swój kąt
            dzięki <em>jednej</em> szczęśliwej obserwacji, która eliminuje
            pozostałe niewiadome. Dla <M tex="q_1" /> tym kluczem jest niezależność
            rzutu poziomego od <M tex="q_2, q_3" />; dla <M tex="q_3" />{" "}
            — niezmiennicza długość przekątnej; dla <M tex="q_2" /> — liniowość
            po wyłuskaniu <M tex="q_3" />; dla orientacji — rozkład grupowy{" "}
            <M tex="R_0^6 = R_0^3 \cdot R_3^6" />. Te cztery obserwacje to{" "}
            cała architektura algorytmu; reszta to mechaniczne stosowanie{" "}
            <code>atan2</code>.
          </p>
          <p>
            Ten sam paradygmat („szukaj kombinacji eliminującej pozostałe
            niewiadome") sprawdza się też w innych analitycznych IK — dla
        ...
        <section className="prose-ik">
          <h2>Osobliwości (singularności) w jednym miejscu</h2>
          <p>
            W poprzednich krokach kilkukrotnie wspominaliśmy konfiguracje, w
            których algorytm staje wobec sytuacji nietypowej —{" "}
            <em>osobliwości</em> (w literaturze angielskojęzycznej i wielu
            polskich tłumaczeniach: <em>singularności</em>). Nie są to błędy
            algorytmu, tylko miejsca
            w przestrzeni konfiguracyjnej, w których manipulator <em>fizycznie</em>{" "}
            traci jeden stopień swobody (kierunek, w którym nie może już
            poruszyć narzędziem, bez zmiany położenia paru przegubów na raz).
            Algebraicznie objawiają się one tym, że formuła traci jednoznaczność:
            układ równań ma albo nieskończenie wiele rozwiązań, albo nie ma żadnego.
          </p>
          <table>
            <thead>
              <tr>
                <th>Typ osobliwości</th>
                <th>Warunek matematyczny</th>
                <th>Co się dzieje fizycznie</th>
                <th>Konsekwencja dla IK</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td><strong>Osobliwość osi 1</strong> (nad bazą)</td>
                <td>
                  <M tex="p_x^2 + p_y^2 = d_3^2" />{" "}
                  (cel na granicy „cylindra zakazanego")
                </td>
                <td>
                  TCP znajduje się dokładnie nad pionową osią bazy — obrót{" "}
                  <M tex="q_1" /> wokół tej osi nie zmienia położenia TCP.
                </td>
                <td>
                  <M tex="q_1" /> staje się nieokreślony (każdy kąt jest
                  równie dobry). Numerycznie pojawia się{" "}
                  <code>atan2(0, 0)</code>, które Python zwraca jako 0 — to
                  <em> arbitralny</em> wybór, nie poprawne rozwiązanie.
                </td>
              </tr>
              <tr>
                <td>
                  <strong>Osobliwość łokcia — wewnętrzna</strong>
                </td>
                <td>
                  <M tex="L^2 = K^2" />, czyli{" "}
                  <M tex="D = |a_2 \pm L|" /> — przekątna bark↔wrist osiąga
                  ekstremum.
                </td>
                <td>
                  Ramię jest maksymalnie wyprostowane{" "}
                  (<M tex="D = a_2 + L" />) lub maksymalnie zgięte{" "}
                  (<M tex="D = |a_2 - L|" />). Trójkąt bark-łokieć-wrist
                  degeneruje się do odcinka.
                </td>
                <td>
                  Dyskryminant <M tex="L^2 - K^2 = 0" /> — dwie gałęzie
                  <em> elbow up</em> i <em>elbow down</em> zlewają się w jedno
                  rozwiązanie. Algorytm zwróci konkretną liczbę, ale „rezerwa"
                  manewrowa łokcia spadła do zera.
                </td>
              </tr>
              <tr>
                <td>
                  <strong>Osobliwość łokcia — zewnętrzna</strong>{" "}
                  (poza zasięgiem)
                </td>
                <td><M tex="L^2 - K^2 < 0" /></td>
                <td>
                  Cel leży <em>poza fizycznym zasięgiem</em> ramienia —
                  trójkąt o bokach <M tex="a_2, L, D" /> nie spełnia
                  nierówności trójkąta (suma dwóch krótszych boków jest
                  mniejsza od trzeciego).
                </td>
                <td>
                  <M tex="\sqrt{\text{ujemna liczba}}" /> → wyjątek
                  numeryczny. Trzeba albo odrzucić pozę celu, albo wybrać
                  drugą gałąź barku (czasem cel jest osiągalny tylko z drugiej
                  strony manipulatora).
                </td>
              </tr>
              <tr>
                <td>
                  <strong>Osobliwość nadgarstka (gimbal lock)</strong>
                </td>
                <td>
                  <M tex="\sin q_5 = 0" />, czyli{" "}
                  <M tex="q_5 \in \{0, \pi\}" />.
                </td>
                <td>
                  Osie <M tex="\hat z_4" /> i <M tex="\hat z_6" /> stają się
                  współliniowe (oba są tą samą prostą) — obrót wokół jednej
                  jest nieodróżnialny od obrotu wokół drugiej. Klasyczna
                  blokada Eulera/Cardano.
                </td>
                <td>
                  Tylko <em>suma</em> <M tex="q_4 + q_6" /> (przy{" "}
                  <M tex="q_5 = 0" />) lub <em>różnica</em> <M tex="q_4 - q_6" />{" "}
                  (przy <M tex="q_5 = \pi" />) jest wyznaczona; sam{" "}
                  <M tex="q_4" /> i <M tex="q_6" /> mają nieskończenie wiele
                  par. Trzeba przyjąć umowę (np. <M tex="q_4 = 0" />) i
                  dopasować <M tex="q_6" /> do reszty.
                </td>
              </tr>
            </tbody>
          </table>
          <p>
            <strong>Wspólny mianownik wszystkich czterech przypadków:</strong>{" "}
            dwie kolumny Jakobianu <M tex="J(q)" /> stają się liniowo zależne,
            więc wyznacznik <M tex="\det J = 0" />. Geometrycznie znaczy to, że
...
function JointRolePanel({
  index, name, axis, analogy, children,
}: {
  index: string;
  name: string;
  axis: string;
  ...
  return (
    <div className="rounded-lg border border-[var(--panel-border)] bg-[var(--panel)] px-4 py-3">
      <div className="flex items-baseline gap-2 mb-1">
        <span className="font-mono font-bold text-base text-[var(--accent,#a855f7)]">{index}</span>
        <span className="text-xs uppercase tracking-wider text-[var(--muted)]">{name}</span>
      </div>
      <div className="text-[11px] text-[var(--muted)] mb-2">
        Oś obrotu: <span className="italic">{axis}</span>
      </div>
      <div className="text-xs italic text-[var(--foreground)] mb-2 leading-snug border-l-2 border-[var(--panel-border)] pl-2">
        {analogy}
      </div>
      <p className="text-xs leading-snug text-[var(--foreground)]">{children}</p>
    </div>
  );
}

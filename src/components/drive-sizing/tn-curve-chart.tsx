...
export function TnCurveChart() {
  ...
  const W = 760, H = 410;
  // Większe paddingi: l=95 dla pełnych etykiet "τ_peak = XX.XX",
  // b=58 żeby etykieta ω_max nie kolidowała z tytułem osi x.
  const pad = { l: 95, r: 20, t: 22, b: 58 };
    ...
    <div className="space-y-3 not-prose">
      ...
      <div className="rounded-lg border border-[var(--panel-border)] bg-white overflow-hidden">
        <svg viewBox={`0 0 ${W} ${H}`} className="w-full">
          ...
          {/* Etykiety regionów — krótsze, żeby nie wystawały poza obszar */}
          ...
          {/* Etykieta qd_max — umieszczona w dodatkowym wierszu pod osią,
              żeby nie kolidowała z tytułem osi x niżej. */}
          ...
          {/* Tytuły osi — y dla osi x przesunięty niżej, x dla osi y bliżej krawędzi viewBox */}
          <text x={pad.l + plotW / 2} y={pad.t + plotH + 44} textAnchor="middle" fontSize={11} fontFamily="monospace" fill="#475569">
            prędkość po przekładni q̇ [rad/s]
          </text>
          <text transform={`rotate(-90 18 ${pad.t + plotH / 2})`}
                x={18} y={pad.t + plotH / 2} textAnchor="middle"
